...
export const renderSelectedList = (): void => {
  ...
  const selectedToyTemplate: ToyTemplateFunction = (toy) => `<div class="selected-toy__wrapper" id="${toy.num}"><img class="selected-toy__image" src="./assets/toys/${toy.num}.webp" alt="${toy.name}"><h3 class="selected-toy__title">${toy.name}</h3><button class="selected-toy__delete">Remove</button></div>
  `;
  if (modalInner !== null) {
    if (stateSelectedList.length === 0) {
      modalInner.innerHTML = ``;
      const emptyMessage: TemplateFunction = () => `<p class="selected-modal__message-empty">
          You have no favourite toys
      </p>`;
